...
export class CrearUsuariosPRDService {
  ...
  createUser(newUser: CompanyUsers_Saphety) {
    return new Promise((resolve, reject) => {
      this._getDataPRD.NewUser(newUser).subscribe(data => {
        if (data['IsValid']) {
  ...
  SearchUser(email: string) {
    return new Promise((resolve, reject) => {
      this._getDataPRD.SearchUser(email).subscribe(data => {
        if (data['IsValid']) {
          resolve(data['ResultData']);
        }
        else {
  ...
  RolesUser(userId: string) {
    return new Promise((resolve, reject) => {
      this._getDataPRD.RolesUser(userId).subscribe(data => {
        if (data['IsValid']) {
          resolve(data['ResultData']);
        }
        else {
  ...
  EditUser(newUser: CompanyUsers_Saphety, userId: string) {
    return new Promise((resolve, reject) => {
      this._getDataPRD.EditUser(newUser, userId).subscribe(data => {
        if (data['IsValid']) {
          resolve(data['ResultData']);
        }
        else {
  ...
  Edit_RolUser(newUser: any, userId: string) {
    return new Promise((resolve, reject) => {
      this._getDataPRD.Edit_RolUser(newUser, userId).subscribe(data => {
        if (data['IsValid']) {
          resolve(data['ResultData']);
        }
        else {
  ...
  SearchcompanyRoles() {
    return new Promise((resolve, reject) => {
      this._getDataPRD.SearchcompanyRoles().subscribe(data => {
        if (data['IsValid']) {
          resolve(data['ResultData']);
        }
        else {
  ...
  Searchcompany(idCompany: string, virtualOperator: string) {
    return new Promise((resolve, reject) => {
      this._getDataPRD.GetCompany(idCompany, virtualOperator).subscribe(data => {
        if (data['IsValid']) {
          resolve(data['ResultData']);
        }
        else {
  ...
  ArmarUsuario(companyU: CompanyR_UC, virtualOperator: string, creation: CompanyCreationLog) {
    ...
    if (companyUser != undefined) {
      ...
      newUser.CompanyMemberships[0].CompanyId = creation.IdCompany_PRD; //IdCompany PRD
        ...
        await this.SearchcompanyRoles()
          .then(async data => {
            ...
            roles.forEach(async rol => {
              let rolPRD = companyRoles_list.find(rolPRD => rolPRD.Name == rol.CompanyRoleName);
              if (rolPRD != undefined) {
                rol.CompanyRoleId = rolPRD.Id;
                newroles.push(rol);
              }
            });
