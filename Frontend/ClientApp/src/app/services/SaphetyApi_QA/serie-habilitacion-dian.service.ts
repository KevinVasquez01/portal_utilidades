...
export class SerieHDianService {
  ...
  createSerieD(serieHDian: SeriesHDian, virtualOperator: string, companyId: string) {
    return new Promise((resolve, reject) => {
      this._getGetDataQA.NewSerieHDian(serieHDian, virtualOperator, companyId).subscribe(data => {
        if (data['IsValid']) {
  ...
  updateSerieD(serieHDian: SeriesHDian, virtualOperator: string, companyId: string, idSerie: string) {
    return new Promise((resolve, reject) => {
      this._getGetDataQA.UpdateSerieHDian(serieHDian, virtualOperator, companyId, idSerie).subscribe(data => {
        if (data['IsValid']) {
          resolve(data['ResultData']);
        }
        else {
  ...
  SearchSerieHD(virtualOperator: string, companyId: string) {
    return new Promise<any>((resolve, reject) => {
      this._getGetDataQA.SearchSerieHDian(virtualOperator, companyId).subscribe(data => {
        if (data['IsValid']) {
          resolve(data['ResultData']);
        }
        else {
  ...
  async CrearSerieHDian(companyU: CompanyR_UC, virtualOperator: string) {
    ...
    if (companyU.series.length == 0) {
      ...
      if (creation.IdCompany_QA != '') {
        await this.SearchSerieHD(virtualOperator, creation.IdCompany_QA)
          .then(async result => {
            ...
            if (seriesH.length > 0) {
              ...
              await this.updateSerieD(serieHDian, virtualOperator, creation.IdCompany_QA, idSerie)
                .then(async () => {
              ...
              await this.createSerieD(serieHDian, virtualOperator, creation.IdCompany_QA)
