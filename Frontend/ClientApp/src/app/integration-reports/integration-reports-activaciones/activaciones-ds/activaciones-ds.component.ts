...
import { ReportsHistory_JsonDS } from 'src/app/models/Reports/reports-history';
...
@Component({
  selector: 'app-activaciones-ds',
  templateUrl: './activaciones-ds.component.html',
  styleUrls: ['./activaciones-ds.component.scss'],
})
export class ActivacionesDSComponent implements OnInit {
  opvs_QA: Voperator[] = [];
  opvs_PRD: Voperator[] = [];
  opv_Not_Apply = 'saphety';
  AlliesNames_List: Array<AlliesNames> = []; //Lista de valores de Aliados

  companies_DS_Act: companiesFinal_Saphety[] = []; //Empreas obtenidas API Saphety
  newCompanies_DS: companiesFinal_Saphety[] = []; //Nuevas empresas activas en DS
  ...
  Companies_reported_Salesforce_DS_List: CompaniesReported_SalesForce[] = []; //Empresas reportadas en SalesForce
  Last_time_activationsReport_generation_DS: Date = new Date(); //Ultima vez que se generó reporte
  ...
  //Armado tablas
  ReportDS_SalesPerson: string = '';
  ReportDS_OWNER: string = '';
  ReportDS_OPNAME_Text: string = '';
  ReportDS_PNAME_Text: string = '';

  CreateSupportDocumentOpp_Table: CreateSupportDocumentOpp[] = []; //Opp
  CreateSupportDocumentProject_Table: CreateSupportDocumentProject[] = []; //Project
  ...
  async awaitDataElements() {
    ...
    await this._reportsHistoryU
      .GetCompaniesReportedSalesforce_ByType('supportdocument_activations')
      .then((result) => {
        this.Companies_reported_Salesforce_DS_List = JSON.parse(
    ...
    await this._dataElementsU
      .getDdataelementU('Last_time_activationsReport_generation_DS')
      .then((result) => {
        this.Last_time_activationsReport_generation_DS = JSON.parse(
          result.json
        );
      })
      .catch((error) => this.Getting_data_errors.push(JSON.stringify(error)));
  ...
  SearhCompanies_Response(
    ...
    let find = this.companies_DS_Act.find(
      (x) => x.DocumentNumber == company.Identification.DocumentNumber
    );
    //Solo agrega empresa a lista si tiene DS activo y no existe en listado
    if (company.SupportDocumentIssuingServiceActive && find === undefined) {
      ...
      this.companies_DS_Act.push(company_final);
    } else {
  ...
  want_Run_Report() {
    const dialog = this._dialog.open(IntegrationReportsDialogComponent, {
      width: '450px',
      data: {
        titulo: `Generar informe Activaciones DS`,
        mensaje: `¿Desea generar informe Activaciones Facturación Electrónica?`,
        respuesta: false,
      },
    });
  ...
  async Run_Report() {
    ...
    //Obtiene ReportDS_SalesPerson de UtilidadesAPI
    await this._dataElementsU
      .getDdataelementU('ReportDS_SalesPerson')
      .then((result) => {
        //Desde servicio Utilidades
        this.ReportDS_SalesPerson = result.json;
      })
      .catch((error) => {
        this.errorsAPI.push({
          message: `Ocurrió un error al obtener ReportDS_SalesPerson`,
          json: JSON.stringify(error),
        });
        general_result = false;
        return;
    ...
    await this._dataElementsU
      .getDdataelementU('ReportDS_OWNER')
      .then((result) => {
        //Desde servicio Utilidades
        this.ReportDS_OWNER = result.json;
      })
      .catch((error) => {
        this.errorsAPI.push({
          message: `Ocurrió un error al obtener ReportDS_OWNER`,
          json: JSON.stringify(error),
        });
        general_result = false;
        return;
    ...
    await this._dataElementsU
      .getDdataelementU('ReportDS_OPNAME_Text')
      .then((result) => {
        //Desde servicio Utilidades
        this.ReportDS_OPNAME_Text = result.json;
      })
      .catch((error) => {
        this.errorsAPI.push({
          message: `Ocurrió un error al obtener ReportDS_OPNAME_Text`,
          json: JSON.stringify(error),
        });
        general_result = false;
        return;
    ...
    //Obtiene ReportDS_PNAME_Text de UtilidadesAPI
    await this._dataElementsU
      .getDdataelementU('ReportDS_PNAME_Text')
      .then((result) => {
        //Desde servicio Utilidades
        this.ReportDS_PNAME_Text = result.json;
      })
      .catch((error) => {
        this.errorsAPI.push({
          message: `Ocurrió un error al obtener ReportDS_PNAME_Text`,
          json: JSON.stringify(error),
        });
        general_result = false;
        return;
      ...
      this.opvs_PRD.map(async (opv) => {
        ...
        if (
          CompanyActivationPlans.find(
            (x) => x.Type == 'SupportDocumentCompanyActivation'
          ) != undefined
        ) {
    ...
    this.newCompanies_DS = [];
    await Promise.all(
      this.companies_DS_Act.map(async (result) => {
        let find = this.Companies_reported_Salesforce_DS_List.find(
          (x) => x.document_number == result.DocumentNumber
        );
        if (find === undefined) {
          this.newCompanies_DS.push(result);
        }
      })
    );
  ...
  async BuildWidgets() {
    this.WD_total_companies_SalesForce =
      this.Companies_reported_Salesforce_DS_List.length;

    try {
      let now = this._datePipe.transform(new Date(), 'dd/MM/yyyy');
      let last = this._datePipe.transform(
        this.Last_time_activationsReport_generation_DS,
        'dd/MM/yyyy'
      );
    ...
    //Arma data
    await Promise.all(
      this.newCompanies_DS.map(async (company) => {
        //Activaciones Número
          ...
          this.WD_company_money.push({
            ...
            value:
              valor_Aliado === undefined
                ? 0
                : valor_Aliado.Support_Document_Value,
          });
        } else {
          ...
          (foundMoney.value =
            valor_Aliado === undefined
              ...
              : Number(foundMoney.value) +
                Number(valor_Aliado.Support_Document_Value)),
  ...
  ProjectOppLength(i: number, isProject: boolean, maxLength: number): string {
    let toreturn = '';
    if (isProject) {
      let part1 = `${this.ReportDS_PNAME_Text}${this.newCompanies_DS[i].Alias_opv.toUpperCase()}_`;
      let part2 = `_${this.newCompanies_DS[i].DocumentType}_${this.newCompanies_DS[i].DocumentNumber}`;

      //Si quedan menos de 28 caracteres para nombre, corta alias OV
      let sobrante = maxLength - (part1.length + part2.length);
      if(sobrante < 28){
        let Alias_opv = this.newCompanies_DS[i].Alias_opv.substring(0,this.newCompanies_DS[i].Alias_opv.length > 15 ? 15 : this.newCompanies_DS[i].Alias_opv.length);
        part1 = `${this.ReportDS_PNAME_Text}${Alias_opv.toUpperCase()}_`;
        sobrante = maxLength - (part1.length + part2.length);
      }

      let nameCompany = this.newCompanies_DS[i].Name.substring(0, this.newCompanies_DS[i].Name.length > sobrante ? sobrante : this.newCompanies_DS[i].Name.length);
      toreturn = `${part1}${nameCompany}${part2}`;
    } else {
      let part1 = this.ReportDS_OPNAME_Text;
      let part2 = `_${this.newCompanies_DS[i].DocumentType}_${this.newCompanies_DS[i].DocumentNumber}_${this.newCompanies_DS[i].Alias_opv.toUpperCase()}`;

      //Si quedan menos de 68 caracteres para nombre, corta alias OV
      let sobrante = maxLength - (part1.length + part2.length);
      if(sobrante < 68){
        let Alias_opv = this.newCompanies_DS[i].Alias_opv.substring(0,this.newCompanies_DS[i].Alias_opv.length > 15 ? 15 : this.newCompanies_DS[i].Alias_opv.length);
        part2 = `_${this.newCompanies_DS[i].DocumentType}_${this.newCompanies_DS[i].DocumentNumber}_${Alias_opv.toUpperCase()}`;
        sobrante = maxLength - (part1.length + part2.length);
      }
      let nameCompany = this.newCompanies_DS[i].Name.substring(0, this.newCompanies_DS[i].Name.length > sobrante ? sobrante : this.newCompanies_DS[i].Name.length);
      toreturn = `${part1}${nameCompany}${part2}`;
  ...
  async BuildTables() {
    ...
    this.CreateSupportDocumentOpp_Table = [];
    for (let i = 0; i < this.newCompanies_DS.length; i++) {
      let valor_Aliado = this.AlliesNames_List.find(
        (x) =>
          x.OV_Name.toLocaleLowerCase() ==
          this.newCompanies_DS[i].Alias_opv.toLocaleLowerCase()
      );

      let newOpp: CreateSupportDocumentOpp = {
        'OV.Column1.Name': this.newCompanies_DS[i].Name_opv,
        'OPPORTUNITY NAME': this.ProjectOppLength(i,false,120),
        'Column1.Identification.DocumentNumber':
          this.newCompanies_DS[i].DocumentNumber,
        PRICE:
          valor_Aliado === undefined
            ? Number(0)
            : Number(valor_Aliado.Support_Document_Value),
        'CLOSE DATE': new Date(fechaInforme),
        'SALES PERSON': this.ReportDS_SalesPerson,
      };
      this.CreateSupportDocumentOpp_Table.push(newOpp);
    }

    //Arma data CreateProject_Table
    this.CreateSupportDocumentProject_Table = [];
    for(let i = 0; i<this.newCompanies_DS.length; i++){
      let newProject: CreateSupportDocumentProject = {
        'OPPORTUNITY NAME': this.ProjectOppLength(i,false,120),
        'PROJECT NAME': this.ProjectOppLength(i,true,80),
        OWNER: this.ReportDS_OWNER,
        ...
        '% Excecution': 100,
        'Project Type': 'ActivacionDS',
      };
      this.CreateSupportDocumentProject_Table.push(newProject);
    }
    ...
    await this.SaveReport()
      .then(async () => {
        ...
        await this.UpdateCompanies_reported_Salesforce_DS();
  ...
  async UpdateLastTime_Report() {
    await this._dataElementsU.updateLast_time_activationsReport_generation_DS(
      new Date()
    );
  }

  //Actualiza Tabla maestra de compañías ya reportadas a SalesForce
  async UpdateCompanies_reported_Salesforce_DS() {
    let fechaInforme = new Date(
    ...
    const newCompanies_reported_Salesforce_DS: CompaniesReported_SalesForce[] =
      [];

    let userfullname = localStorage.getItem('userfullname');
    await Promise.all(
      this.newCompanies_DS.map(async (company) => {
        newCompanies_reported_Salesforce_DS.push({
          date_activation: fechaInforme,
          document_number: company.DocumentNumber,
          report_type: 'supportdocument_activations',
          user: userfullname != undefined ? userfullname : '',
        });
      })
    );
    await this._reportsHistoryU.NewCompaniesReportedSalesforce(
      newCompanies_reported_Salesforce_DS
    );
  }
  ...
  async SaveReport() {
    ...
    let json: ReportsHistory_JsonDS = {
      jsonOPP: JSON.stringify(this.CreateSupportDocumentOpp_Table),
      jsonPROJECT: JSON.stringify(this.CreateSupportDocumentProject_Table),
    };
    ...
    await this._reportsHistoryU.NewReportsHistory({
      ...
      new_companies: this.newCompanies_DS.length,
