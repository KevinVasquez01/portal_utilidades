...
@Component({
  selector: 'app-activaciones-fe',
  templateUrl: './activaciones-fe.component.html',
  styleUrls: ['./activaciones-fe.component.scss'],
})
export class ActivacionesFEComponent implements OnInit {
  opvs_QA: Voperator[] = [];
  opvs_PRD: Voperator[] = [];
  opv_Not_Apply = 'saphety';
  AlliesNames_List: Array<AlliesNames> = []; //Lista de valores de Aliados

  companies_FE_Act: companiesFinal_Saphety[] = []; //Empreas obtenidas API Saphety
  newCompanies_FE: companiesFinal_Saphety[] = []; //Nuevas empresas activas en FE
  ...
  Companies_reported_Salesforce_FE_List: CompaniesReported_SalesForce[] = []; //Empresas reportadas en SalesForce
  Last_time_activationsReport_generation_FE: Date = new Date(); //Ultima vez que se generó reporte
  ...
  //Armado tablas
  ReportFE_SalesPerson: string = '';
  ReportFE_OWNER: string = '';
  ReportFE_OPNAME_Text: string = '';
  ReportFE_PNAME_Text: string = '';

  CreateOpp_Table: CreateInvoiceOpp[] = []; //Opp
  CreateProject_Table: CreateInvoiceProject[] = []; //Project
  ...
  async awaitDataElements() {
    ...
    await this._reportsHistoryU
      .GetCompaniesReportedSalesforce_ByType('invoice_activations')
      .then((result) => {
        this.Companies_reported_Salesforce_FE_List = JSON.parse(
    ...
    await this._dataElementsU
      .getDdataelementU('Last_time_activationsReport_generation_FE')
      .then((result) => {
        this.Last_time_activationsReport_generation_FE = JSON.parse(
          result.json
        );
      })
      .catch((error) => this.Getting_data_errors.push(JSON.stringify(error)));
  ...
  SearhCompanies_Response(
    ...
    let find = this.companies_FE_Act.find(
      (x) => x.DocumentNumber == company.Identification.DocumentNumber
    );
    //Solo agrega empresa a lista si tiene FE activa y no existe en listado
    if (company.InvoiceIssuingServiceActive && find === undefined) {
      ...
      this.companies_FE_Act.push(company_final);
    } else {
  ...
  want_Run_Report() {
    const dialog = this._dialog.open(IntegrationReportsDialogComponent, {
      width: '450px',
      data: {
        titulo: `Generar informe Activaciones FE`,
  ...
  async Run_Report() {
    ...
    //Obtiene ReportFE_SalesPerson de UtilidadesAPI
    await this._dataElementsU
      .getDdataelementU('ReportFE_SalesPerson')
      .then((result) => {
        //Desde servicio Utilidades
        this.ReportFE_SalesPerson = result.json;
      })
      .catch((error) => {
        this.errorsAPI.push({
          message: `Ocurrió un error al obtener ReportFE_SalesPerson`,
          json: JSON.stringify(error),
        });
        general_result = false;
        return;
    ...
    //Obtiene ReportFE_OWNER de UtilidadesAPI
    await this._dataElementsU
      .getDdataelementU('ReportFE_OWNER')
      .then((result) => {
        //Desde servicio Utilidades
        this.ReportFE_OWNER = result.json;
      })
      .catch((error) => {
        this.errorsAPI.push({
          message: `Ocurrió un error al obtener ReportFE_OWNER`,
          json: JSON.stringify(error),
        });
        general_result = false;
        return;
    ...
    //Obtiene ReportFE_OPNAME_Text de UtilidadesAPI
    await this._dataElementsU
      .getDdataelementU('ReportFE_OPNAME_Text')
      .then((result) => {
        //Desde servicio Utilidades
        this.ReportFE_OPNAME_Text = result.json;
      })
      .catch((error) => {
        this.errorsAPI.push({
          message: `Ocurrió un error al obtener ReportFE_OPNAME_Text`,
          json: JSON.stringify(error),
        });
        general_result = false;
        return;
    ...
    //Obtiene ReportFE_PNAME_Text de UtilidadesAPI
    await this._dataElementsU
      .getDdataelementU('ReportFE_PNAME_Text')
      .then((result) => {
        //Desde servicio Utilidades
        this.ReportFE_PNAME_Text = result.json;
      })
      .catch((error) => {
        this.errorsAPI.push({
          message: `Ocurrió un error al obtener ReportFE_PNAME_Text`,
          json: JSON.stringify(error),
        });
        general_result = false;
        return;
    ...
    this.newCompanies_FE = [];
    await Promise.all(
      this.companies_FE_Act.map(async (result) => {
        let find = this.Companies_reported_Salesforce_FE_List.find(
          (x) => x.document_number == result.DocumentNumber
        );
        if (find === undefined) {
          this.newCompanies_FE.push(result);
        }
      })
    );
  ...
  async BuildWidgets() {
    this.WD_total_companies_SalesForce =
      this.Companies_reported_Salesforce_FE_List.length;

    try {
      let now = this._datePipe.transform(new Date(), 'dd/MM/yyyy');
      let last = this._datePipe.transform(
        this.Last_time_activationsReport_generation_FE,
        'dd/MM/yyyy'
      );
    ...
    //Arma data
    await Promise.all(
      this.newCompanies_FE.map(async (company) => {
        //Activaciones Número
          ...
          this.WD_company_money.push({
            ...
            value:
              valor_Aliado === undefined
                ? 0
                : valor_Aliado.Invoice_Activation_Value,
          });
        } else {
          ...
          (foundMoney.value =
            valor_Aliado === undefined
              ...
              : Number(foundMoney.value) +
                Number(valor_Aliado.Invoice_Activation_Value)),
   ...
   ProjectOppLength(i: number, isProject: boolean, maxLength: number): string {
    let toreturn = '';
    if (isProject) {
      let part1 = `${this.ReportFE_PNAME_Text}${this.newCompanies_FE[i].Alias_opv.toUpperCase()}_`;
      let part2 = `_${this.newCompanies_FE[i].DocumentType}_${this.newCompanies_FE[i].DocumentNumber}`;

      //Si quedan menos de 28 caracteres para nombre, corta alias OV
      let sobrante = maxLength - (part1.length + part2.length);
      if(sobrante < 28){
        let Alias_opv = this.newCompanies_FE[i].Alias_opv.substring(0,this.newCompanies_FE[i].Alias_opv.length > 15 ? 15 : this.newCompanies_FE[i].Alias_opv.length);
        part1 = `${this.ReportFE_PNAME_Text}${Alias_opv.toUpperCase()}_`;
        sobrante = maxLength - (part1.length + part2.length);
      }

      let nameCompany = this.newCompanies_FE[i].Name.substring(0, this.newCompanies_FE[i].Name.length > sobrante ? sobrante : this.newCompanies_FE[i].Name.length);
      toreturn = `${part1}${nameCompany}${part2}`;
    } else {
      let part1 = this.ReportFE_OPNAME_Text;
      let part2 = `_${this.newCompanies_FE[i].DocumentType}_${this.newCompanies_FE[i].DocumentNumber}_${this.newCompanies_FE[i].Alias_opv.toUpperCase()}`;

      //Si quedan menos de 68 caracteres para nombre, corta alias OV
      let sobrante = maxLength - (part1.length + part2.length);
      if(sobrante < 68){
        let Alias_opv = this.newCompanies_FE[i].Alias_opv.substring(0,this.newCompanies_FE[i].Alias_opv.length > 15 ? 15 : this.newCompanies_FE[i].Alias_opv.length);
        part2 = `_${this.newCompanies_FE[i].DocumentType}_${this.newCompanies_FE[i].DocumentNumber}_${Alias_opv.toUpperCase()}`;
        sobrante = maxLength - (part1.length + part2.length);
      }
      let nameCompany = this.newCompanies_FE[i].Name.substring(0, this.newCompanies_FE[i].Name.length > sobrante ? sobrante : this.newCompanies_FE[i].Name.length);
      toreturn = `${part1}${nameCompany}${part2}`;
  ...
  async BuildTables() {
    ...
    for (let i = 0; i < this.newCompanies_FE.length; i++) {
      let valor_Aliado = this.AlliesNames_List.find(
        (x) =>
          x.OV_Name.toLocaleLowerCase() ==
          this.newCompanies_FE[i].Alias_opv.toLocaleLowerCase()
      );

      let newOpp: CreateInvoiceOpp = {
        'OV.Column1.Name': this.newCompanies_FE[i].Name_opv,
        'OPPORTUNITY NAME': this.ProjectOppLength(i,false,120),
        'Column1.Identification.DocumentNumber':
          this.newCompanies_FE[i].DocumentNumber,
        PRICE:
          valor_Aliado === undefined
            ? Number(0)
            : Number(valor_Aliado.Invoice_Activation_Value),
        'CLOSE DATE': new Date(fechaInforme),
        'SALES PERSON': this.ReportFE_SalesPerson,
      };
    ...
    for(let i = 0; i<this.newCompanies_FE.length; i++){
      let newProject: CreateInvoiceProject = {
        'OPPORTUNITY NAME': this.ProjectOppLength(i,false,120),
        'PROJECT NAME': this.ProjectOppLength(i,true,80),
        OWNER: this.ReportFE_OWNER,
    ...
    await this.SaveReport()
      .then(async () => {
        ...
        await this.UpdateCompanies_reported_Salesforce_FE();
  ...
  async UpdateLastTime_Report() {
    await this._dataElementsU.updateLast_time_activationsReport_generation_FE(
      new Date()
    );
  }

  //Actualiza Tabla maestra de compañías ya reportadas a SalesForce
  async UpdateCompanies_reported_Salesforce_FE() {
    let fechaInforme = new Date(
    ...
    await Promise.all(
      this.newCompanies_FE.map(async (company) => {
        newCompanies_reported_Salesforce_FE.push({
          ...
          report_type: 'invoice_activations',
  ...
  async SaveReport() {
    ...
    await this._reportsHistoryU.NewReportsHistory({
      ...
      new_companies: this.newCompanies_FE.length,
